...
class Member_list extends MX_Controller
{
    ...
    public function index($id = null)
    {
        ...
        $data['title']    = display('member_list');
        #-------------------------------#       
        #
        #pagination starts
        #
        $config["base_url"] = base_url('setting/member_list/index');
        $config["total_rows"]  = $this->member_model->countlist();
        $config["per_page"]    = 25;
        ...
        $data["memberlist"] = $this->member_model->read($config["per_page"], $page);
        $data["links"] = $this->pagination->create_links();

        if (!empty($id)) {
            $data['title'] = display('edit_member');
            $data['intinfo']   = $this->member_model->findById($id);
        }
        #
        #pagination ends
        #   
    ...
    public function create($id = null)
    {
        ...
        $data['title'] = display('add_town');
        #-------------------------------#
        $this->form_validation->set_rules('town', display('townname'), 'required|max_length[50]');
        $saveid = $this->session->userdata('id');
        $data['type']   = (object) $postData = array(
            'townid'          => $this->input->post('townid'),
            'townname'          => $this->input->post('town', true),
            'status'             => 1,
        );
        $data['intinfo'] = "";
        if ($this->form_validation->run()) {
            if (empty($this->input->post('countryid'))) {
                $this->permission->method('setting', 'create')->redirect();
                $logData = array(
                    'action_page'         => display('country_list'),
    ...
    public function createcity($id = null)
    {
        ...
        if ($this->form_validation->run()) {
            if (empty($this->input->post('cityid'))) {
                ...
                redirect("setting/town_city_list/citylist");
            } else {
                $this->permission->method('setting', 'update')->redirect();
                $postData2 = array(
                    'cityid'              => $this->input->post('cityid'),
                ...
                if ($this->towncity_model->updatecity($postData2)) {
                    $this->logs_model->log_recorded($logData);
                    $this->session->set_flashdata('message', display('update_successfully'));
                } else {
                    $this->session->set_flashdata('exception',  display('please_try_again'));
            ...
            if (!empty($id)) {
                ...
                $data['intinfo']   = $this->towncity_model->findByIdcity($id);
            }
    ...
    public function updatecityfrm($id)
    {
        ...
        $data['title'] = display('edit_city');
        $data['intinfo']   = $this->towncity_model->findByIdcity($id);
        $data['allstate']   = $this->towncity_model->allstate();
        $data['module'] = "setting";
        $data['page']   = "cityedit";
    ...
    public function deletecity($id = null)
    {
        ...
        if ($this->towncity_model->deletecity($id)) {
            #Store data to log table.
            $this->logs_model->log_recorded($logData);
            #set success message
            $this->session->set_flashdata('message', display('delete_successfully'));
